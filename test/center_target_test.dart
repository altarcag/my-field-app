...
void main() {
  test('distance uses meters and takes the short path across the dateline', () {
    ...
    expect(distanceToMapCenter(const LatLng(0, 0), const LatLng(0, 0.001)), closeTo(111.2, 1));
    expect(distanceToMapCenter(const LatLng(0, 179.999), const LatLng(0, -179.999)), closeTo(222.4, 2));
  });

  testWidgets('target stays centered and distance updates when panning and GPS moves', (tester) async {
    ...
    await tester.pumpWidget(MaterialApp(home: Scaffold(body: StatefulBuilder(
      builder: (context, setState) {
        ...
        return FlutterMap(mapController: controller,
          ...
          children: [CenterTargetLayer(position: gps, menuOpen: menu,
            onToggleMenu: () => setState(() => menu = !menu),
            onCreate: (kind, center) { selected = kind; chosen = center; },
          )],
        );
      },
    ))));
    ...
    update(() { gps = const LatLng(0, 0.001); menu = false; });
    await tester.pump();
    expect(find.text('0 m'), findsOneWidget);
    update(() => gps = null);
    await tester.pump();
  ...
  testWidgets('log dialog validates text and can reopen after dismissal', (tester) async {
    FieldLogDraft? draft;
    await tester.pumpWidget(MaterialApp(home: Builder(builder: (context) => Scaffold(
      body: TextButton(onPressed: () async {
        draft = await showDialog<FieldLogDraft>(context: context, builder: (_) => const FieldLogDialog(
          kind: FieldLogKind.text, projectName: 'Test project', latitude: 40, longitude: 28,
        ));
